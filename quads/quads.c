...
struct big_block* descend_expr_ast(ast_node* node, int* tmp_ctr) {
    ...
    switch (node->type) {
        ...
        case (AST_funct):
            ...
            // Todo: Add support for functs to know how many arguments they have
            struct list_node* args = funct_el->args;
            // Might need to make this a pointer or something
            int* counter = malloc(sizeof(int));
            counter = 0;

            struct gen_node_t* arg_count = new_const(counter);
            quad* argBegin = quad_gen(NULL, counter, NULL, Q_ARGBEGIN);
            ...
            while (args->cur != NULL) {
                ...
                quad* arg_quad = quad_gen(NULL, current_arg, counter, Q_ARG);
                append(ret, arg_quad);
            }
            break;
